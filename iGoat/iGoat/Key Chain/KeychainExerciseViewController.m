...
- (void)viewDidLoad
{
    ...
    if (_switch_userDefault.isOn && _switch_keychain.isOn) {
        _switch_keychain.on = NO;
        _switch_userDefault.on = YES;
    }
}
...
- (void)storeCredentialsInSettingsApp
{
    ...
    _alertMessage = @"Stored in NSUserDefaults";
}
...
- (void)storeCredentialsInKeychain
{
    ...
    if (results == errSecSuccess) {
        ...
        if (results == errSecSuccess) {
            ...
            _alertMessage = @"Updated in keychain.";

        } else {
            _alertMessage = @"Exists in keychain, but error updating it.";
        }
        ...
        _alertMessage = @"Added in keychain.";
    } else {
        _alertMessage = @"Error adding/updating in keychain.";
    }
}
...
{
    _switch_keychain.on = !sender.isOn;
}

- (IBAction)keychainSwitchDidClick:(UISwitch *)sender {
    _switch_userDefault.on = !sender.isOn;
}
